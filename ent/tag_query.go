...
func (tq *TagQuery) QueryArticleTags() *ArticleTagQuery {
	query := &ArticleTagQuery{config: tq.config}
...
func (tq *TagQuery) Clone() *TagQuery {
	...
	return &TagQuery{
		...
		withArticleTags: tq.withArticleTags.Clone(),
...
func (tq *TagQuery) WithArticleTags(opts ...func(*ArticleTagQuery)) *TagQuery {
	query := &ArticleTagQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withArticleTags = query
	return tq
}
...
func (tq *TagQuery) sqlAll(ctx context.Context) ([]*Tag, error) {
	...
	if query := tq.withArticleTags; query != nil {
		...
		query.Where(predicate.ArticleTag(func(s *sql.Selector) {
			s.Where(sql.InValues(tag.ArticleTagsColumn, fks...))
		}))
