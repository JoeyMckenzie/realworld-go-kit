...
func (ar *articlesRepository) CreateArticle(ctx context.Context, userId int, title, slug, description, body string, tagIdList []int) (*ArticleEntity, error) {
    var article ArticleEntity

    tagListToInsert := make([]int32, len(tagIdList))
    for _, tagId := range tagIdList {
        tagListToInsert = append(tagListToInsert, int32(tagId))
    }

    const sql = `
INSERT INTO public.articles (created_at, updated_at, title, slug, description, body, user_id, tags)
VALUES (current_timestamp, current_timestamp, $1::VARCHAR, $2::VARCHAR, $3::VARCHAR, $4::VARCHAR, $5::INTEGER, $6)
RETURNING *`

    if err := ar.db.GetContext(ctx, &article, sql, title, slug, description, body, userId, pq.Array(tagListToInsert)); err != nil {
        return nil, err
    }
