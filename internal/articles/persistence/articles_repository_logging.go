...
func (mw *articlesRepositoryLoggingMiddleware) GetFeedArticles(ctx context.Context, request *domain.GetArticlesServiceRequest) (articles *[]ArticleEntity, err error) {
	defer func(begin time.Time) {
		level.Info(mw.logger).Log(
			"method", "GetFeedArticles",
			"exec_time", time.Since(begin),
			"found", fmt.Sprint(articles != nil),
			"error", fmt.Sprint(err != nil),
		)
...
func (mw *articlesRepositoryLoggingMiddleware) UserHasFavoritedArticle(ctx context.Context, userId, articleId int) (favorited bool) {
	defer func(begin time.Time) {
		level.Info(mw.logger).Log(
			"method", "UserHasFavoritedArticle",
			"exec_time", time.Since(begin),
			"favorited", favorited,
			"user_id", userId,
		)
	...
	return mw.next.UserHasFavoritedArticle(ctx, userId, articleId)
}
