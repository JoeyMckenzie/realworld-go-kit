...
func (mw *usersServiceLoggingMiddleware) Update(ctx context.Context, request users.AuthenticationRequest[users.UpdateUserRequest]) (user *users.User, err error) {
    defer func(begin time.Time) {
        level.Info(mw.logger).Log(
            "method", "Update",
            "request_time", time.Since(begin),
