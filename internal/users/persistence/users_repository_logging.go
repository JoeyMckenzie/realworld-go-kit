...
func (mw *usersRepositoryLoggingMiddleware) IsFollowingUser(ctx context.Context, followerUserId, followeeUserId int) (following bool) {
	defer func(begin time.Time) {
		level.Info(mw.logger).Log(
	...
	return mw.next.IsFollowingUser(ctx, followerUserId, followeeUserId)
}
